...
t2 <- tidy_physeq  %>% group_by(Sample) %>% mutate(Sample_rel_abund = Abundance / sum(Abundance)) %>% #relative abundance of each otu per sample
  ...
  ungroup() %>% 
  group_by(description, g_species) %>% 
...
t2 <- read_csv("../../Analysis/tidy_data_NIOZ140.csv")
...
Prot <- Phyla %>% filter(Phylum %in% c("Proteobacteria", "Bacteroidetes", "Firmicutes", "Actinobacteria", "Cyanobacteria"))
...
Order <- read_csv("../../Analysis/order.csv")
...
Genus <- read_csv( "../../Analysis/genus.csv")
...
oil <-  t2 %>%  filter (Genus %in% oil_deg) %>% distinct() 
#write_csv(oil, "../../Analysis/oil_degraders_tab.csv")
unique(oil$Genus)

oil_ecreme <- oil %>% select(treatment, timepoint, Material, description, Genus, Genus_rep_rel_abund, st_dev_Genus_abund )%>%
  distinct()
head(oil_ecreme)
#write_csv(oil_ecreme, "../../Analysis/genus_oil_degraders.csv")
...
physeq_object = merge_phyloseq(asv, tax, map)                 
...
summarize_phyloseq(physeq_object)
alpha_tab <-microbiome::alpha(physeq_object, index = "all")
write.csv(alpha_tab, file = "./alpha_div/alpha_div_indexes_microbiome_asv.csv")
metad <- data.frame(physeq_object@sam_data) 
metad$Shannon <- alpha_tab$diversity_shannon 
metad$evenness_simpson <- alpha_tab$evenness_simpson 
m <- subset_samples(physeq_object, timepoint %in% c("T1", "T6"))
...
a <- phyloseq::estimate_richness(physeq_object, measures = c("Observed", "Chao1", "ACE", "Shannon", "Simpson", "InvSimpson"))
write.csv(a, file = "./alpha_div/alpha_div_indexes_phyloseq_asv.csv")
plot <- plot_richness(physeq_object, "Material", "treatment", measures="Shannon")+facet_grid(treatment~timepoint)
plot + geom_boxplot(data=plot$data, aes(Material,value,color=NULL), alpha=0.3)+ labs(title = "Alpha Diversity", subtitle ="Shannon", x =NULL , y = NULL )+theme_light()
...
tidy_physeq_asv$LinkerPrimerSequence <- NULL
tidy_physeq_asv$ReversePrimerSequence <- NULL
tidy_physeq_asv$InputFileName <- NULL
tidy_physeq_asv$BarcodeSequence <- NULL
tidy_physeq_asv$BarcodeSequence_1 <- NULL

ASV <- tidy_physeq_asv %>%   unite("Species", Genus:Species, sep= " ", remove = FALSE) %>% 
  mutate(Species = ifelse(str_detect(ASV$Species, c("unassigned")) , "unassigned", Species))  %>% 
  group_by(Sample) %>% mutate(Sample_rel_abund = Abundance / sum(Abundance)) %>% #relative abundance of each otu per sample
  ungroup() %>%
  group_by(description) %>% mutate( rep_rel_abund = Sample_rel_abund / sum(Sample_rel_abund)) %>% #relative abundance of each otu per number of samples in replicates
  ungroup() %>% 
  #Domain_section
...
###########Ranks_tables_ASV#####################
Domain_asv <- ASV  %>%  select(timepoint,treatment, Material, description, Domain, Domain_rep_rel_abund,Domain_st_dev_abund_samples)%>% 
  distinct() 
write_csv(Domain_asv, "./data/domain_asv.csv")

Domain_asv <- Domain_asv %>% filter ( timepoint %in% c("T1", "T6")) #%>% mutate(timepoint = ifelse(Material =="negative_c", "T1", timepoint))

#colors_vector_to_personalize#
CPCOLS <- c("#199442", "#ED1F1F", "#F5EE2C", "#B636D6", "#3D68E0", "#EBA53D", "#00688B", "#00EE76", "#CD9B9B", "#00BFFF", "#FFF68F", "#FF7F50", "#68228B", "#ADFF2F", "#CD0000", "#0000FF", "#CD9B1D", "#FF34B3", "#BBFFFF", "#191970") 

ggplot(Domain_asv, aes(x=Material, y= Domain_rep_rel_abund, fill=Domain))+
  geom_bar(stat="identity", position="stack")+ scale_fill_manual(values = CPCOLS)+
  theme_classic2()+  facet_grid (timepoint~treatment)


#creating table phyla
Phyla_asv <- ASV %>% select(treatment, timepoint, Material,description, Phylum, Phyla_rep_rel_abund ,st_dev_Phylum_abund)%>% 
  distinct() 
head(Phyla_asv)
write_csv(Phyla_asv, "./data/Phyla_asv.csv")

Phyla_asv <- Phyla_asv %>% mutate(Phylum = ifelse(Phyla_rep_rel_abund<0.01, "others<0.01", Phylum)) %>%
  filter ( timepoint %in% c("T1", "T6")) #%>% mutate(timepoint = ifelse(Material =="negative_c", "T1", timepoint))


m2 <- ggplot(Phyla_asv, aes(x=Material, y= Phyla_rep_rel_abund, fill=Phylum)) +
  geom_bar(stat="identity", position="stack")+ scale_fill_manual(values = CPCOLS) +
  theme_classic2()+  facet_grid (timepoint~treatment)+
  labs(title = "ASV", subtitle = NULL, x =NULL , y = NULL )+
  theme (axis.text.x = element_text(face="bold"), axis.text.y = element_text(face="bold") ) 


Prot_asv <- Phyla_asv %>% filter(Phylum %in% c("Proteobacteria", "Bacteroidota", "Firmicutes", "Cyanobacteria"))

p <- ggplot(Prot_asv, aes(x=description, y= Phyla_rep_rel_abund, fill=Phylum)) +
  geom_col(stat="identity", position="dodge")+ scale_fill_manual(values = CPCOLS) +
  theme_pubclean()+ theme(axis.text.x = element_text(angle = 60, hjust = 1))+
  labs(title = "ASV", subtitle = NULL, x =NULL , y = NULL ) +ylim(0,1)+
  geom_errorbar(aes(ymin=Phyla_rep_rel_abund, ymax=Phyla_rep_rel_abund+st_dev_Phylum_abund), width=.2,
                position=position_dodge(.9))
p
...
Class_asv <- ASV %>% 
  select(treatment, timepoint, Material, description, Class, Class_rep_rel_abund, st_dev_Class_abund )%>% 
  distinct()  
head(Class_asv)
write_csv(Class_asv, "./data/Class_asv.csv")

Class_asv <- Class_asv %>% mutate(Class = ifelse(Class_rep_rel_abund<0.01, "others<0.01", Class)) %>%
  filter ( timepoint %in% c("T1", "T6"))

ggplot(Class_asv, aes(x=Material, y= Class_rep_rel_abund, fill=Class))+
  geom_bar(stat="identity", position="stack")+ scale_fill_manual(values = CPCOLS)+
  theme_classic()+  facet_grid (timepoint~treatment)

Order_asv <- ASV %>% select(treatment, timepoint, Material, description, Order, Order_rep_rel_abund, st_dev_Order_abund )%>% 
  distinct() #
head(Order_asv)
write_csv(Order_asv, "./data/Order_asv.csv")

Order_asv <- Order_asv %>% mutate(Order = ifelse(Order_rep_rel_abund<0.01, "others<0.01", Order)) %>% 
  filter ( timepoint %in% c("T1", "T6"))


ggplot(Order_asv, aes(x=Material, y= Order_rep_rel_abund, fill=Order))+
  geom_bar(stat="identity", position="stack")+ scale_color_brewer()+ theme_classic()+  facet_grid (timepoint~treatment)

Family_asv <- ASV %>% select(treatment, timepoint, Material, description, Family, Family_rep_rel_abund, st_dev_Family_abund )%>% 
  distinct()
head(Family_asv)
write_csv(Family_asv, "./data/Family_asv.csv")

Family_asv <- Family_asv %>%  mutate(Family = ifelse(Family_rep_rel_abund<0.01, "others<0.01", Family)) %>% 
  filter ( timepoint %in% c("T1", "T6"))

ggplot(Family_asv, aes(x=Material, y= Family_rep_rel_abund, fill=Family))+
  geom_bar(stat="identity", position="stack")+ scale_color_brewer()+
  theme_classic()+  facet_grid (timepoint~treatment)

Genus_asv <- ASV %>% select(treatment, timepoint, Material, description, Genus, Genus_rep_rel_abund, st_dev_Genus_abund )%>%
  distinct()
head(Genus_asv)
write_csv(Genus_asv, "./data/Genus_asv.csv")

Genus_asv <- Genus_asv %>% mutate(Genus = ifelse(Genus_rep_rel_abund<0.01, "others<0.01", Genus))  %>% 
  filter ( timepoint %in% c("T1", "T6"))

ggplot(Genus_asv, aes(x=Material, y= Genus_rep_rel_abund, fill=Genus))+
  geom_bar(stat="identity", position="stack")+ scale_color_brewer()+ 
  theme_classic()+  facet_grid (timepoint~treatment)

Species_asv <- ASV %>% select(treatment, timepoint, Material, description, Species, Species_rep_rel_abund, st_dev_Species_abund)%>% 
  distinct()

head(Species_asv)
write_csv(Species_asv, "./data/Species_asv.csv") 

Species_asv <- Species_asv %>%  mutate(Species = ifelse(Species_rep_rel_abund<0.02, "others<0.02", Species))  %>% 
  filter ( timepoint %in% c("T1", "T6"))

ggplot(Species_asv, aes(x=Material, y= Species_rep_rel_abund, fill=Species))+
  geom_bar(stat="identity", position="stack")+ scale_fill_manual(values = CPCOLS)+
  theme_classic()+  facet_grid (timepoint~treatment)+ 
  theme (axis.text.x = element_text(face="bold"), axis.text.y = element_text(face="bold") ) 
...
otu_138Table<-read_tsv("./data/otuTable_noSingletons_138_no_1st_line.txt")
tax <- otu_138Table$taxonomy
tax <- read_csv2(tax, col_names = c("Domain", "Phylum", "Class", "Order", "Family", "Genus", "Species"),)
tax <- as.data.frame(tax) 
row.names(tax) <- otu_138Table$`#OTU ID`
tax <- tax %>%  as.matrix() %>% tax_table()
otu_138 <- otu_138Table[, names(otu_138Table) != "taxonomy"] %>% column_to_rownames("#OTU ID") %>%  as.matrix() 
otu_138 <- otu_table(otu_138, taxa_are_rows = T)
map <- sample_data(read.delim("./data/mapping_file_details.txt", row.names = 1, na.strings = c("NA", "")))
physeq_object <- merge_phyloseq(otu_138, tax, map)                 
...
summarize_phyloseq(physeq_object)
alpha_tab <-microbiome::alpha(physeq_object, index = "all")
write.csv(alpha_tab, file = "./alpha_div/alpha_div_indexes_microbiome_138.csv")
metad <- data.frame(physeq_object@sam_data) 
metad$Shannon <- alpha_tab$diversity_shannon 
metad$evenness_simpson <- alpha_tab$evenness_simpson 
m <- subset_samples(physeq_object, timepoint %in% c("T1", "T6"))
...
a <- phyloseq::estimate_richness(physeq_object)
write.csv(a, file = "./alpha_div/alpha_div_indexes_phyloseq_138.csv")
plot <- plot_richness(m, "Material", "treatment", measures="Chao1")+facet_grid(treatment~timepoint)
plot + geom_boxplot(data=plot$data, aes(Material,value,color=NULL), alpha=0.3)+ labs(title = "Alpha Diversity", subtitle ="Chao1", x =NULL , y = NULL )+theme_light()
...
Domain <- t2  %>%  select(timepoint,treatment, Material, description, Domain, Domain_rep_rel_abund,Domain_st_dev_abund_samples)%>% 
  distinct() 
write_csv(Domain, "./data/domain_138.csv")
...
Phyla <- t2 %>% select(treatment, timepoint, Material,description, Phylum, Phyla_rep_rel_abund ,st_dev_Phylum_abund)%>% 
  distinct() 
head(Phyla)
write_csv(Phyla, "./data/phyla_138.csv")

Phyla <- read_csv("./data/phyla_138.csv")

Phyla <- Phyla %>% mutate(Phylum = ifelse(Phyla_rep_rel_abund<0.01, "others<0.01", Phylum)) %>%
  filter ( timepoint %in% c("T1", "T6")) #%>% mutate(timepoint = ifelse(Material =="negative_c", "T1", timepoint))


m <- ggplot(Phyla, aes(x=Material, y= Phyla_rep_rel_abund, fill=Phylum))+
  geom_bar(stat="identity", position="stack")+ scale_fill_manual(values = CPCOLS)+
  theme_classic2()+  facet_grid (timepoint~treatment)+
  theme (axis.text.x = element_text(face="bold"), axis.text.y = element_text(face="bold") ) 


Prot <- Phyla %>% filter(Phylum %in% c("Proteobacteria", "Bacteroidota", "Firmicutes", "Actinobacteria", "Cyanobacteria"))
...
Class <- t2 %>% select(treatment, timepoint, Material, description, Class, Class_rep_rel_abund, st_dev_Class_abund )%>% 
  distinct()  
head(Class)
write_csv(Class, "./data/class_138.csv")
...
Order <- t2 %>% select(treatment, timepoint, Material, description, Order, Order_rep_rel_abund, st_dev_Order_abund )%>% 
  distinct() #
head(Order)
write_csv(Order, "./data/order_138.csv")
...
Family <- t2 %>% select(treatment, timepoint, Material, description, Family, Family_rep_rel_abund, st_dev_Family_abund )%>% 
  distinct()
head(Family)
write_csv(Family, "./data/family_138.csv")
...
Genus <- t2 %>% select(treatment, timepoint, Material, description, Genus, Genus_rep_rel_abund, st_dev_Genus_abund )%>%
  distinct()
head(Genus)
write_csv(Genus, "./data/genus_138.csv")
...
head(Species)
write_csv(Species, "./data/species_138.csv") 
...
#######################filter_asv's_in_at_least_3_samples#########################################################
condition <- function(x) { sum(x > 0) >= 3 }
taxaToKeep <- filter_taxa(physeq_object, condition)
...
tidy_physeq$BarcodeSequence <- NULL
tidy_physeq$BarcodeSequence_1 <- NULL
tidy_physeq$g_species <-
  str_c(tidy_physeq$Genus, " ", tidy_physeq$Species)
...
#write_csv(t3, "../../Analysis/tidy_data_NIOZ140_at_least_3.csv")  

t3 <- read_csv("../../Analysis/tidy_data_NIOZ140_at_least_3.csv")


##############tables for each rank#############

Kingdom <- t3  %>%  select(timepoint,treatment, Material, description, Kingdom, Kingdom_rep_rel_abund,Kingdom_st_dev_abund_samples)%>% 
  distinct() 
#write_csv(Kingdom, "../../Analysis/Kingdom.csv")
...
#creating table phyla
Phyla <- t3 %>% select(treatment, timepoint, Material,description, Phylum, Phyla_rep_rel_abund ,st_dev_Phylum_abund)%>% 
  distinct() 
head(Phyla)
write_csv(Phyla, "../../Analysis/phyla_at_least_3.csv")

Phyla <- read_csv("../../Analysis/phyla_at_least_3.csv")
...
########################differential reads analysis ANCOM-BC###########################################
pseq_genus <- phyloseq::tax_glom(physeq_object, taxrank = "Genus")
out = ancombc(
  phyloseq = pseq_genus, 
  formula = "timepoint", 
  p_adj_method = "fdr", 
